...
import {MapBottomMenu} from '../Components/MapBottomMenu'
import { Marker } from "react-native-maps";

import { useGetRestaurantByIdQuery } from '../api/apiSlice'
import i18n from 'i18next'

export const RestarauntOnMapScreen = (props) => {
    const {
      data: Place,
      isSuccess,
      isLoading,
  ...
  useEffect(() => { (async () => { let { status } = await Location.requestForegroundPermissionsAsync(); if (status !== 'granted') { setErrorMsg('Permission to access location was denied'); return; } if (Place) { const data = await Geocoder.from(Place.latitude, Place.longitude) 
    ...
    return (
      <View style={styles.container}>
        ...
        >
        {isError && <View><Text>Error: {error.message}</Text></View>}
